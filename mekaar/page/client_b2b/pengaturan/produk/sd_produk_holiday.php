                ...
                <div class="widget box">
                    <div class="widget-header">
                        ...
                        <div class="toolbar no-padding">
                            ...
                            <select id="produk" class="btn-group form-control" style="width:auto">
                                <option disabled selected value="">Pilih Produk</option>
<?php
$query = mysqli_query($conection, 'SELECT ma2001.ID_KEY,PROD_ID,PROD_NAMA FROM ma2001 WHERE PROD_LEVEL=0');
while ($data = mysqli_fetch_array($query)) {
    echo '<option value="' . $data['ID_KEY'] . '">' . $data['PROD_NAMA'] . ' </option>';
}
?>
                            </select>
                            <div class="btn-group">
                                <button id="cek_hari" class="btn btn-xs btn-primary"><i class="icon-plus"></i>Tambah Tanggal Libur</button>
                            </div>
                        </div>
                    </div>
                                ...
                                <form id="form-tanggal" class="form-horizontal">
                                    <div class="col-md-12">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea type="text" id="ket" name="ket" class="form-control" required></textarea>
                                        </div>
...
$(document).ready(function(){
    ...
    $('#cek_hari').on('click',function(){
        if($('#produk').val()=="" || $('#produk').val()==null){
            alert('Pilih Produk Terlebih Dahulu');
        }else{
            App.blockUI($("#content"));
            var produk=$('#produk').val();
            $.ajax({
            url : "ambil_data.php?type=libur_produk_hari",
            data: {kode:produk},
            type: "post",
            dataType:"json",
    ...
    function callAjax(r,calendare){
        if($('#produk').val() != ""){
            var produk=$('#produk').val();
            $.ajax({
            url : "ambil_data.php?type=libur_produk_kalender",
            type: "post",
            data: {unix:r,kode:produk},
            dataType:"json",
            success:function(result){
                console.log(result);
                if(result.status){
    ...
    function simpan(obj,str){
        ...
        if(confirm(que)){
            ...
            formData.set("kode",produk);
            formData.set("type","holiday_produk_"+str);
            formData.set("produk",$('#produk').val());
    ...
    function hapus(id){
        if(confirm("Hapus Tanggal?")){
            ...
            formData.set("kode2",produk);
            formData.set("type","holiday_produk_tanggal_hapus");
            formData.set("produk",$('#produk').val());
            formData.set("token", newToken);
